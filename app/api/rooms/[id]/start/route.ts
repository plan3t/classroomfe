...
import { ensureTeacherOwnsRoom, startRoom } from '@/src/lib/room-service';
...
export async function POST(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    await ensureTeacherOwnsRoom(id, session.user.id);
    const room = await startRoom(id);
    emitRoomStarted(room.joinId, room.status, room.startedAt ?? null);
